...
export class InventoryService {
  ...
  async create(createInventoryDto: CreateInventoryDto, currentUser: any) {
    ...
    await this.cacheService.del('cache:market:feed_all_none');
    
    return savedLot;
  }

  async getMarketFeed(query: any) {
    const { cropType, search } = query;
    
    
    const cacheKey = `cache:market:feed_${cropType || 'all'}_${search || 'none'}`;
    ...
    if (search) {
      queryBuilder.andWhere('LOWER(inventory.title) LIKE LOWER(:search)', { search: `%${search}%` });
    }
